...
router.get('/:id?', auth, async (req, res) => {
  try {
    const payrollId = req.params.id || req.user._id;
    ...
    const user = await User.findById(payrollId).select('employeeId email profile.firstName profile.lastName profile.salary');
    if (!user) {
...
router.put('/:id', auth, isAdmin, async (req, res) => {
  try {
    ...
    res.json({ message: 'Salary structure updated successfully', salary: user.profile.salary });
  } catch (error) {
...
router.get('/', auth, isAdmin, async (req, res) => {
  try {
    const users = await User.find({})
      .select('employeeId email profile.firstName profile.lastName profile.salary')
